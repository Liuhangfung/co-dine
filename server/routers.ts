...
👨‍🍳 **烹飪步驟**:
${analysis.steps.map((step: any, idx: number) => `${idx + 1}. ${step.instruction}${step.duration ? ` (${step.duration}分鐘)` : ''}`).join('\n')}
...
## 🍎 健康升級方案（必須嚴格執行）
1. **食材替代建議**（必須包含以下項目）：
   - ✅ **白砂糖 → 蜜糖/生果**：所有白砂糖必須用蜜糖或新鮮生果代替（如蘋果泥、香蕉泥、椰棗）
   - ✅ **鹽 → 香菇粉**：至少50%的鹽改用自製香菇粉（乾香菇磨粉，提供製作方法）
   - ✅ **精製產品 → 天然食材**：白米改糙米、白麵粉改全麥粉、白糖改黃糖/蜜糖
   - ✅ **增加生果**：在食譜中加入至少1-2種新鮮水果，增加天然甜味和纖維
   - 其他健康替代方案（說明用量和益處）
...
3. **健康烹飪方法**${analysis.title.includes('炸') ? '\n   - **🔥 必須：改用氣炸鍋（200°C 15分鐘），減少80%油脂，提供噴油技巧和詳細步驟**' : '\n   - 優先使用蒸、烤、氣炸等低油烹飪方式'}
   - 減少油脂和精製調味料的使用
   - 保留食材營養的烹飪技巧
